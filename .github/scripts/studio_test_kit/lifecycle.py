...
re-uses an existing clone), checks out the branch, then runs the platform's
local installer with UNSLOTH_STUDIO_HOME exported.
...
def install_studio(
    ...
    """Clone (or re-use) the repo at `branch`, then run its local installer.
    ...
    if os.name == "nt":
        ...
        install_cmd = [
            ...
            "-File", str(installer), "--local",
        ]
    else:
        installer = repo / "install.sh"
        install_cmd = ["bash", str(installer), "--local"]
    if not installer.exists():
        raise FileNotFoundError(f"Studio installer missing at {installer}")
    _run(
        install_cmd,
        cwd=repo,
        env={
...
def _find_unsloth_bin(install: StudioInstall) -> str:
    ...
    for candidate in (
        ...
        install.home / ".venv_t5_550" / "Scripts" / "unsloth.exe",
        install.home / ".venv_t5_530" / "Scripts" / "unsloth.exe",
...
def launch_studio(
    ...
    if os.name == "nt":
        # No WSL/Git-Bash dependency on Windows: the native installer produces
        # unsloth.exe, and a detached process can write the same combined log
        # directly. Closing this handle is safe after Popen duplicates it.
        with log_path.open("a", encoding="utf-8") as log_handle:
            proc = subprocess.Popen(
                [bin_path, "studio", "-p", str(port)],
                cwd=install.repo,
                env={**os.environ, **env},
                stdout=log_handle,
    ...
    # Best-effort PID capture via `pgrep -f` on POSIX. Windows already holds
    # the exact Popen PID above.
    if os.name != "nt":
        try:
