...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as workdir:
        ...
        hardlink_root = Path(workdir) / "hardlink-scan"
        for directory_index in range(10):
            directory = hardlink_root / f"d{directory_index}"
            directory.mkdir(parents = True)
            for file_index in range(50):
                (directory / f"f{file_index}.txt").write_text("sandbox benchmark\n")
        sandbox._assert_no_external_hardlinks(str(hardlink_root))
        hardlink_scan = distribution_ms(
            lambda: sandbox._assert_no_external_hardlinks(str(hardlink_root)),
            30,
        )
    ...
    results = {
        ...
        "hardlink_scan_500_files": hardlink_scan,
    }
